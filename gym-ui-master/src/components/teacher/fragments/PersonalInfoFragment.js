...
class PersonalInfoFragment extends React.Component {
    ...
    componentDidMount() {
        axiosInstance.get('/teacher/myInformation').then(res => {
            this.setState({
                name: res.data.name,
    ...
    render() {
        ...
        const handleEditButton = () => {
            if (this.state.editing) {
                axiosInstance.post('/teacher/updateInformation', {
                ...
                <Paper className={classes.paper}>
                    ...
                    <Box className={classes.padding2}>
                        ...
                        <Grid container spacing={3}>
                            <Grid item xs={3} md={2} lg={1}>
                                <Typography
                                    className={clsx(classes.formFieldLabel, classes.formParagraph)}>性别</Typography>
                            </Grid>
                            <Grid item xs={9} md={10} lg={11}>{
                                this.state.editing ? (
                                    <TextField fullWidth={true} margin="none" value={this.state.newGender}
                                               onChange={(event) => {
                                                   this.setState({newGender: event.target.value})
                                               }}/>
                                ) : (<Typography className={classes.formParagraph}>{this.state.gender}</Typography>)
                            }</Grid>
                        </Grid>
                        <Grid container spacing={3}>
                            <Grid item xs={3} md={2} lg={1}>
